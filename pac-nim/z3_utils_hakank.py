...
def makeIntVectorMatrix(sol, name, rows, cols, min_value, max_value):
    ...
    for i in range(rows):
        for j in range(cols):
            x[(i, j)] = makeIntVar(sol, name + "%i_%i" %
...
def bin_packing(sol, capacity, bins, weights):
    n = len(bins)
    for b in range(n):
        sol.add(Sum([weights[j]*If(bins[j] == b, 1, 0)
                for j in range(n)]) <= capacity)
...
def regular(sol, x, Q, S, d, q0, F, x_len):
    ...
    d2_flatten_a = makeIntArray(sol, "d2_flatten_a_%i" % uuid.uuid4().int, len(
        d2_flatten), min(d2_flatten), max(d2_flatten))
    ...
    a = [makeIntVar(sol, 'a[%i]_%i' % (i, uuid.uuid4().int), 0, Q + 1)
...
def regular2(sol, x, Q, S, d, q0, F, x_len):
    ...
    for i in x_range:
        ...
        # Determine a[i+1]: a[i+1] == d2[a[i], x[i]]
        element(sol, (a[i] * S) + (x[i] - 1),
                d2_flatten, a[i + 1], d2_flatten_len)
...
def nvalue(sol, m, x, min_val, max_val):
    n = len(x)
    sol.add(m == Sum([If(Sum([If(x[j] == i, 1, 0) for j in range(n)]) > 0, 1, 0)
...
def all_differ_from_at_least_k_pos(sol, k, vectors):
    ...
    for i in range(n):
        for j in range(i+1, n):
            sol.add(Sum([If(vectors[i][kk] != vectors[j][kk], 1, 0)
                    for kk in range(m)]) >= k)
    ...
    while sol.check() == sat:
        ...
        sol.add(
            Or(
                ...
                cc != c, Or([gcc[i] != gccs[i] for i in range(n)]),
                # , vv != v
            )
        )
